  ...
  while (true) {
	  ...
	  for (const auto & t: tasks) {
		  ...
		  case task::can_read:
			  ...
			  if (s1[s].can_read() != s2[s].can_read()) {
				  log_info() << "Can read failure" << std::endl;
				  return EXIT_FAILURE;
			  }
			  break;
		  case task::read: {
			  ...
			  for (size_t i=0; i < count; ++i) {
				  if (s1[s].read() != s2[s].read()) {
					  log_info() << "Read failure" << std::endl;
					  return EXIT_FAILURE;
				  }
			  }
			  break;
		  ...
		  case task::get_offset:
			  ...
			  if (s1[s].offset() != s2[s].offset()) {
				  log_info() << "Offset failure" << std::endl;
				  return EXIT_FAILURE;
			  }
		  case task::get_size: break;
			  log_info() << "size" << std::endl;
			  if (f1.size() != f2.size()) {
				  log_info() << "Size failure" << std::endl;
				  return EXIT_FAILURE;
			  }
		  }
		  break;
